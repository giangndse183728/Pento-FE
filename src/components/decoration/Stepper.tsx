...
function StepIndicator({ step, currentStep, onClickStep, disableStepIndicators }: StepIndicatorProps) {
  ...
  return (
    <motion.div onClick={handleClick} className="step-indicator" animate={status} initial={false}>
      <motion.div
        variants={{
          inactive: { scale: 1, backgroundColor: '#F7FBFC', color: '#113F67' },
          active: { scale: 1, backgroundColor: '#769FCD', color: '#769FCD' },
          complete: { scale: 1, backgroundColor: '#769FCD', color: '#769FCD' }
        }}
...
function StepConnector({ isComplete }: StepConnectorProps) {
  const lineVariants: Variants = {
    incomplete: { width: 0, backgroundColor: '#ffff' },
    complete: { width: '100%', backgroundColor: '#769FCD' }
  };
