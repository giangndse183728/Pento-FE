...
#define DITHER_STRENGTH 1.0
    ...
    float span=smoothstep(-10.0,0.0,y)*(1.0-smoothstep(R_V-20.0,R_V+50.0,y));
    return uWIntensity*sum*topF*bGain*span;
}

void mainImage(out vec4 fc,in vec2 frag){
    vec2 C=iResolution.xy*.5; float invW=1.0/max(C.x,1.0);
    float sc=512.0/iResolution.x*.3;
...
export const LaserFlow: React.FC<Props> = ({
  ...
  return <div ref={mountRef} className="absolute inset-0 w-full h-screen top-0 left-[-20%]  "  style={style} />;
};
