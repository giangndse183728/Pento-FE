...
import { RecipeSummary } from '../../schema/recipeSchema';
            ...
            <div className="grid gap-3 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {items.map((recipe) => (
                    <WhiteCard
                        ...
                        width="100%"
                        height="auto"
                        className="group h-full border border-white/80 shadow-lg hover:shadow-xl transition-shadow duration-200 relative cursor-pointer"
                        onClick={() => router.push(`/admin/recipes/${recipe.id}`)}
                    >
                        {deleteMode && (
                            <button
                                onClick={(e) => handleDeleteClick(recipe.id, e)}
                                className="absolute top-2 right-2 z-10 w-7 h-7 rounded-full bg-red-500 hover:bg-red-600 text-white shadow-lg flex items-center justify-center transition-all duration-200 hover:scale-110"
                        ...
                        <div className="flex flex-col gap-3">
                            ...
                            <div className="relative flex items-start justify-between gap-4">
                                <div className="min-w-0">
                                    <p
                                        className="text-[10px] font-semibold uppercase tracking-[0.2rem] mb-1"
                                        style={{ color: ColorTheme.blueGray }}
                                    >
                                        Recipes of
                                    </p>
