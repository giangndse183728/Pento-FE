...
import { colorTheme, palettes } from '@/constant/colorTheme';
...
const defaultColors = palettes.coolBlue;
  ...
  useEffect(() => {
    ...
    class MouseClass {
      ...
      init(container: HTMLElement) {
        ...
        // Keep hover detection on the container only (not critical for interaction latency)
      ...
      onDocumentMouseMove(event: MouseEvent) {
        ...
        // Compute immediate diff to avoid one-frame latency
        if (this.container) {
          const rect = this.container.getBoundingClientRect();
          const prev = this.coords.clone();
          const nx = (event.clientX - rect.left) / rect.width;
          const ny = (event.clientY - rect.top) / rect.height;
          this.coords.set(nx * 2 - 1, -(ny * 2 - 1));
          this.diff.subVectors(this.coords, prev);
          this.coords_old.copy(prev);
          this.hasImmediateEvent = true;
          this.mouseMoved = true;
        } else {
      ...
      update() {
        ...
        // If we just processed an input event, keep its instantaneous diff for this frame
        if (this.hasImmediateEvent) {
          this.hasImmediateEvent = false;
          this.coords_old.copy(this.coords);
        } else {
